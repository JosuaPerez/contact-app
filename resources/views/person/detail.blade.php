                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-6 gap-x-6 gap-y-6">
                        <div class="sm:col-span-3">
                            <div class="flex items-center justify-between">
                                ...
                                <a href="{{ route('person.index') }}" class="">Go Back</a>
                            </div>
                            <dl>
                                ...
                                <dd class="pl-3">
                                    @foreach ($person->tags as $tag)
                                        <span class="bg-green-600 text-xs px-1 rounded-full">{{$tag->tag_name}}</span>
                                    @endforeach
                                </dd>
                            </dl>

                            <div class="pt-3">
                                <a class="py-2 px-3 rounded-full"
                                   href="{{route('person.edit', $person->id)}}">Edit Person</a>
                            </div>
                        </div>
                        <div class="sm:col-span-3">
                            ...
                            <form action="{{ route('task.store') }}" method="POST">
                                ...
                                <div class="mt-5 flex items-center justify-end gap-x-6">
                                    <button type="submit" class="flex items-center justify-end ml-2 py-2 px-3 rounded-full">Create Task</button>
                                </div>
                            </form>
                                    ...
                                    @if ($task->status == "open")
                                        <div class="pt-3">
                                            <form action="{{route('task.complete', $task->id)}}" method="POST">
                                                @csrf
                                                @method('PUT')
                                                <button class="py-2 px-3 rounded-full" type="submit">Complete Task</button>
                                            </form>
                                        </div>
                                    @else
